...
public class TranslationViewDragHelper {
     ...
     */
    public static TranslationViewDragHelper create(ViewGroup forParent, Callback cb) {
        return new TranslationViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static TranslationViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        final TranslationViewDragHelper helper = create(forParent, cb);
    ...
    private boolean forceSettleCapturedViewAt(int finalLeft, int finalTop, int xvel, int yvel) {
        final int startLeft = (int) mCapturedView.getX();
        final int startTop = (int) mCapturedView.getY();
    ...
    public void flingCapturedView(int minLeft, int minTop, int maxLeft, int maxTop) {
        ...
        mScroller.fling((int) mCapturedView.getX(), (int) mCapturedView.getY(),
                (int) VelocityTrackerCompat.getXVelocity(mVelocityTracker, mActivePointerId),
                (int) VelocityTrackerCompat.getYVelocity(mVelocityTracker, mActivePointerId),
    ...
    protected boolean canScroll(View v, boolean checkV, int dx, int dy, int x, int y) {
        if (v instanceof ViewGroup) {
            ...
            for (int i = count - 1; i >= 0; i--) {
                ...
                if (x + scrollX >= child.getX() && x + scrollX < child.getX() + child.getWidth() &&
                        y + scrollY >= child.getY() && y + scrollY < child.getY() + child.getHeight() &&
                        canScroll(child, true, dx, dy, (int) (x + scrollX - child.getX()),
                                (int) (y + scrollY - child.getY()))) {
                    return true;
                }
            }
        }
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mDragState == STATE_DRAGGING) {
                    ...
                    dragTo((int) (mCapturedView.getX() + idx), (int) (mCapturedView.getY() + idy), idx, idy);
    ...
    private int getEdgesTouched(int x, int y) {
        ...
        if (x < mParentView.getX() + mEdgeSize) result |= EDGE_LEFT;
        if (y < mParentView.getY() + mEdgeSize) result |= EDGE_TOP;
        if (x > (mParentView.getX() + mParentView.getWidth()) - mEdgeSize) result |= EDGE_RIGHT;
        if (y > (mParentView.getY() + mParentView.getHeight()) - mEdgeSize) result |= EDGE_BOTTOM;
